...
`lien <chap-13.html#version5_architecture>`__ nous avions remarqué que
rien dans la classe implémentant la couche **[dao]** ne laissait
supposer qu’on utilisait un SGBD particulier. C’est ce que nous allons
vérifier maintenant en utilisant un autre SGBD, le SGBD PostgreSQL.
L’architecture en couches devient la suivante :
...
paragraphe `lien <chap-03.html#config_php>`__) :
...
`lien <chap-14.html#installation_postgres>`__). Puis de la même façon
que vous avez lancé l’outil **[services]**, lancez l’outil **[pgadmin]**
qui permet d’administrer le SGBD PostgreSQL **[1-3]** :
...
`lien <chap-13.html#version5_remplissage_table>`__. Il nous suffit de
modifier le fichier **[database.json]** qui décrit la base de données :
...
`lien <chap-13.html#version5_calcul_impot>`__. Il nous suffit de
reprendre le script
...
-  lignes 9-28 : définition de l’environnement du test. Nous utilisons
   ...
   `lien <chap-13.html#version5_calcul_script_principal>`__ ;
...
-  lignes 9-28 : définition de l’environnement du test. Nous utilisons
   ...
   **[MainCalculateImpotsWithTaxAdminDataInPostgresDatabase]** décrit au
   paragraphe `lien <chap-14.html#version6_calcul_impot>`__ ;
...
.. |image38| image:: ./chap-14/media/image39.png
   :width: 5.64173in
   :height: 1.25984in
.. |image39| image:: ./chap-14/media/image40.png
   :width: 5.72008in
   :height: 1.43346in
