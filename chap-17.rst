...
C'était le but du paragraphe `lien <chap-16.html#réseau_http>`__.

Le client web décrit au paragraphe
`lien <chap-16.html#réseau_http_exemple6>`__ nous a permis de découvrir
une partie du protocole HTTP.
...
`lien <chap-16.html#réseau_http>`__. Avec le navigateur Firefox (mai
2019), il faut faire F12 pour avoir accès à ces données :
...
`lien <chap-23.html#version12>`__. Ce chapitre s’intéresse plutôt aux
**services web** : des pages PHP qui délivrent, via un serveur web, des
données à destination d’autres clients PHP. Néanmoins il nous a semblé
utile de donner quelques rudiments de HTML au lecteur.
...
moment présent au paragraphe `lien <chap-17.html#web_page_dynamique>`__.
C’était le script **[exemple-01.php]** suivant :
...
Au paragraphe `lien <chap-16.html#réseau_http>`__ nous avons développé
...
`lien <chap-02.html#laragon>`__), on tape la commande suivante :
...
paragraphe `lien <chap-16.html#réseau_utilitaires_tcp>`__) :
...
un terminal Laragon (cf paragraphe `lien <chap-02.html#laragon>`__) :
...
terminal Laragon (cf paragraphe `lien <chap-02.html#laragon>`__) :
...
paragraphe `lien <chap-03.html#config_php>`__). Dans un environnement
...
`lien <chap-17.html#web_sessions_exemple1_client>`__). On ne modifie que
son fichier de configuration **[config-session-client]** :
...
**[http-02.php]** du paragraphe
`lien <chap-16.html#réseau_http_exemple6>`__. Ce script utilisait la
bibliothèque **[libcurl]** pour se connecter à des sites HTTP et HTTPS.
On avait alors utilisé la configuration suivante pour cette
bibliothèque :
